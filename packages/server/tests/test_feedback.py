...
class TestBugReport:
    def test_requires_auth(self, client):
        resp = client.post("/api/bug", json={"body": "Something broke badly"})
    ...
    def test_db_persistence(self, client):
        ...
        client.post("/api/bug", json={"body": body}, headers=auth_header(key))
...
class TestQuestion:
    def test_requires_auth(self, client):
        resp = client.post("/api/question", json={"body": "How do I raise properly?"})
        assert resp.status_code == 401
    ...
    def test_db_persistence(self, client):
        ...
        client.post("/api/question", json={"body": body}, headers=auth_header(key))
