...
def seed_game(game_client) -> tuple[int, str, str]:
    ...
    gid = game_client.post("/api/games", json={"game_type": "poker", "max_players": 2}).json()["game_id"]
...
class TestLobby:
    ...
    def test_list_games_after_create(self, game_client, client):
        game_client.post("/api/games", json={"game_type": "poker", "max_players": 2})
        resp = client.get("/api/games")
    ...
    def test_game_page_exists(self, game_client, client):
        game_client.post("/api/games", json={"game_type": "poker"})
...
class TestStreamsRead:
    ...
    def test_list_all_streams(self, game_client, client):
        gid = game_client.post("/api/games", json={"game_type": "poker"}).json()["game_id"]
        key = game_module.account_store.create_account("Alice")
