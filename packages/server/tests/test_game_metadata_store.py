...
class TestGameMetadataStore:
    def test_create_and_list(self):
        ...
        store.create(game_id=1, mode="offchain", buy_in=100, max_players=2)
        rows = store.list_all()
        assert len(rows) == 1
    ...
    def test_exists(self):
        ...
        store.create(game_id=1, mode="offchain", buy_in=0, max_players=0)
        assert store.exists(1) is True

    def test_update_player_joined(self):
        store = GameMetadataStore(get_pool())
        store.create(game_id=1, mode="offchain", buy_in=0, max_players=0)
        store.update_player_joined(1, "Alice")
        store.update_player_joined(1, "Bob")
        rows = store.list_all()
    ...
    def test_update_game_over(self):
        store = GameMetadataStore(get_pool())
        store.create(game_id=1, mode="offchain", buy_in=0, max_players=0)
        store.update_game_over(1, "Alice")
        rows = store.list_all()
    ...
    def test_update_funded(self):
        store = GameMetadataStore(get_pool())
        store.create(game_id=1, mode="onchain", buy_in=100, max_players=2)
        store.update_funded(1)
        rows = store.list_all()
    ...
    def test_create_with_optional_fields(self):
        store = GameMetadataStore(get_pool())
        store.create(
            game_id=1, mode="onchain", buy_in=100, max_players=2,
