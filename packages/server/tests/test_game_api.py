...
from core.unified_router import configure as configure_unified_router, _build_spectator_for_game
...
def reset_state():
    ...
    game_module._snapshot_buffer = _snapshot_buffer
...
class TestJoinGame:
    ...
    def test_join_requires_auth(self, client):
        gid = create_game(client)
        resp = client.post(f"/api/games/{gid}/join", json={"wallet_address": None})
        assert resp.status_code == 401

    def test_join_invalid_key(self, client):
        gid = create_game(client)
        resp = client.post(f"/api/games/{gid}/join", json={"wallet_address": None}, headers=auth_header("pk_bogus"))
        assert resp.status_code == 401
    ...
    def test_join_duplicate_name(self, client):
        ...
        resp = client.post(f"/api/games/{gid}/join", json={"wallet_address": None}, headers=auth_header(key))
...
class TestStart:
    def test_start_game(self, client):
        ...
        join_game(client, gid, key_a)
        join_game(client, gid, key_b)
        resp = client.post(f"/api/games/{gid}/start", headers=auth_header(key_a))
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_start_not_enough_players(self, client):
        ...
        key = register_account(client, "Alice")
        join_game(client, gid, key)
        resp = client.post(f"/api/games/{gid}/start", headers=auth_header(key))
        assert resp.status_code == 400

    def test_start_requires_auth(self, client):
        gid = create_game(client)
        resp = client.post(f"/api/games/{gid}/start")
        assert resp.status_code == 401
...
class TestState:
    def _setup_started_game(self, client):
        ...
        join_game(client, gid, key_a)
        join_game(client, gid, key_b)
        client.post(f"/api/games/{gid}/start", headers=auth_header(key_a))
        return gid, key_a, key_b

    def test_state_requires_auth(self, client):
        gid, _, _ = self._setup_started_game(client)
        resp = client.get(f"/api/games/{gid}/state")
        assert resp.status_code == 401

    def test_state_non_player_rejected(self, client):
        gid, _, _ = self._setup_started_game(client)
        key_c = register_account(client, "Charlie")
        resp = client.get(f"/api/games/{gid}/state", headers=auth_header(key_c))
        assert resp.status_code == 403
    ...
    def test_state_before_start(self, client):
        ...
        key = register_account(client, "Alice")
        join_game(client, gid, key)
        resp = client.get(f"/api/games/{gid}/state", headers=auth_header(key))
        assert resp.status_code == 400
...
class TestAction:
    ...
    def _who_acts_first(self, client, gid, key_a, key_b):
        s1 = client.get(f"/api/games/{gid}/state", headers=auth_header(key_a)).json()
        if s1["is_your_turn"]:
            return key_a, key_b
        return key_b, key_a
    ...
    def test_action_requires_auth(self, client):
        gid, _, _ = self._setup_started_game(client)
        resp = client.post(f"/api/games/{gid}/action", json={"action": "fold"})
        assert resp.status_code == 401
    ...
    def test_full_hand_via_api(self, client):
        ...
        for _ in range(20):  # safety limit
            ...
            if s1["is_your_turn"]:
                ...
                s2 = client.get(f"/api/games/{gid}/state", headers=auth_header(key_b)).json()
                if s2["is_your_turn"]:
                    key = key_b
            ...
            if to_call > 0:
                client.post(f"/api/games/{gid}/action", json={"action": "call"}, headers=auth_header(key))
            else:
                client.post(f"/api/games/{gid}/action", json={"action": "check"}, headers=auth_header(key))

        # Hand should have completed
        final = client.get(f"/api/games/{gid}/state", headers=auth_header(key_a)).json()
...
class TestSpectator:
    ...
    def test_spectator_sees_previous_hand_after_completion(self, client):
        ...
        client.post(f"/api/games/{gid}/action", json={"action": "fold"}, headers=auth_header(first_key))
...
class TestActionComments:
    ...
    def test_comment_in_action(self, client):
        ...
        state = client.get(f"/api/games/{gid}/state", headers=auth_header(first_key)).json()
        comments = [a["comment"] for a in state["recent_actions"] if a.get("comment")]
        assert "I'm feeling lucky!" in comments
        # Complete hand
        client.post(f"/api/games/{gid}/action", json={"action": "fold"}, headers=auth_header(second_key))
        spec = client.get(f"/api/games/{gid}/spectator").json()
...
class TestGameIsolation:
    def test_games_are_isolated(self, client):
        ...
        w1 = client.get(f"/api/games/{g1}/waiting").json()
        w2 = client.get(f"/api/games/{g2}/waiting").json()
    ...
    def test_action_on_wrong_game(self, client):
        ...
        client.post(f"/api/games/{g1}/start", headers=auth_header(key_a))
...
class TestChat:
    ...
    def test_chat_requires_auth(self, client):
        gid = create_game(client)
        resp = client.post(f"/api/games/{gid}/chat", json={"message": "Hello"})
        assert resp.status_code == 401
...
class TestTimer:
    ...
    def test_extend_requires_auth(self, client):
        gid, _, _ = self._setup_started_game(client)
        resp = client.post(f"/api/games/{gid}/extend")
        assert resp.status_code == 401

    def test_extend_requires_player(self, client):
        gid, _, _ = self._setup_started_game(client)
        key_c = register_account(client, "Charlie")
        resp = client.post(f"/api/games/{gid}/extend", headers=auth_header(key_c))
        assert resp.status_code == 403
    ...
    def test_extend_wrong_turn(self, client):
        ...
        if s1["is_your_turn"]:
            ...
            wrong_key = key_a
        resp = client.post(f"/api/games/{gid}/extend", headers=auth_header(wrong_key))
        assert resp.status_code == 400
    ...
    def test_extend_success(self, client):
        ...
        resp = client.post(f"/api/games/{gid}/extend", headers=auth_header(right_key))
        assert resp.status_code == 200
        data = resp.json()
...
class TestReason:
    ...
    def test_reason_hidden_from_player_state(self, client):
        ...
        for key in (first_key, second_key):
            state = client.get(f"/api/games/{gid}/state", headers=auth_header(key)).json()
...
class TestStreams:
    ...
    def test_create_stream_requires_auth(self, client):
        gid = create_game(client)
        resp = client.post(f"/api/games/{gid}/streams", json={"title": "No auth"})
        assert resp.status_code == 401
    ...
    def test_stream_commentate_requires_auth(self, client):
        ...
        resp = client.post(f"/api/streams/{sid}/commentate", json={"text": "No auth"})
        assert resp.status_code == 401
    ...
    def test_stream_data(self, client):
        ...
        resp = client.get(f"/api/streams/{sid}/data")
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_stream_data_not_found(self, client):
        resp = client.get("/api/streams/999/data")
        assert resp.status_code == 404
    ...
    def test_multiple_streams_independent_commentary(self, client):
        ...
        client.post(f"/api/streams/{sid_a}/commentate", json={"text": "Alice says hi"}, headers=auth_header(key_a))
        client.post(f"/api/streams/{sid_b}/commentate", json={"text": "Bob says hey"}, headers=auth_header(key_b))

        resp_a = client.get(f"/api/streams/{sid_a}/data")
        resp_b = client.get(f"/api/streams/{sid_b}/data")
    ...
    def test_raw_spectator_has_no_commentary(self, client):
        ...
        client.post(f"/api/streams/{sid}/commentate", json={"text": "Hello!"}, headers=auth_header(key))
...
class TestResign:
    ...
    def test_resign_requires_auth(self, client):
        gid = create_game(client)
        resp = client.post(f"/api/games/{gid}/resign")
        assert resp.status_code == 401
    ...
    def test_resign_requires_player(self, client):
        ...
        resp = client.post(f"/api/games/{gid}/resign", headers=auth_header(key_d))
        assert resp.status_code == 404
    ...
    def test_resign_visible_in_spectator(self, client):
        ...
        client.post(f"/api/games/{gid}/resign", headers=auth_header(key_b))
        spec = client.get(f"/api/games/{gid}/spectator").json()
    ...
    def test_resign_before_start(self, client):
        ...
        key = register_account(client, "Alice")
        join_game(client, gid, key)
        resp = client.post(f"/api/games/{gid}/resign", headers=auth_header(key))
        assert resp.status_code == 400
