...
class TestJoinDebit:
    ...
    def test_insufficient_balance_rejected(self, client):
        ...
        gid = create_offchain_game(client, buy_in=500)
        resp = client.post(f"/game/poker/{gid}/join", json={}, headers=auth_header(key))
        assert resp.status_code == 400
    ...
    def test_join_failure_refunds(self, client):
        ...
        resp = client.post(f"/game/poker/{gid}/join", json={}, headers=auth_header(key_alice))
        assert resp.status_code == 400
...
class TestOffchainLifecycle:
    def test_full_game_with_settlement(self, client):
        ...
        state_data = client.get(f"/game/poker/{gid}/state", headers=auth_header(key_a)).json()
        assert state_data["game_over"] is True

        # Verify game is over
        spectator_resp = client.get(f"/game/poker/{gid}/spectator")
        assert spectator_resp.json()["game_over"] is True

        # Check offchain settlement endpoint
        settlement_resp = client.get(f"/game/poker/{gid}/offchain-settlement")
