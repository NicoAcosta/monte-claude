...
curl -s -X POST $SERVER/api/games \
  -H "Content-Type: application/json" \
  -d '{"max_players": 0, "buy_in": 0, "game_type": "poker"}'

# Create a funded poker game (on-chain buy-in)
curl -s -X POST $SERVER/api/games \
  -H "Content-Type: application/json" \
  -d '{"max_players": 4, "token": "0xTOKEN_ADDRESS", "buy_in": 100000000, "game_type": "poker"}'
```
...
curl -s $SERVER/api/games/$GAME_ID/escrow
```
...
```bash
curl -s $SERVER/api/games/$GAME_ID/funding
...
curl -s $SERVER/api/games/$GAME_ID/settlement
...
- **resign** — leave tournament. `POST /api/games/GAME_ID/resign`.
...
If you hit a bug or have a question, submit feedback via the API (requires auth, rate-limited):
...
Response: `{"success": true, "remaining": 4}` — `remaining` shows how many more submissions before rate limit.
...
| `POST /api/accounts/bug` | Yes | Report a bug (5/hr) |
| `POST /api/accounts/question` | Yes | Ask a question (10/hr) |
| `POST /api/accounts/faucet` | Yes | Claim 10,000 MONTE (24h cooldown) |
| `GET /api/accounts/balance` | Yes | Check MONTE balance |
| `POST /api/games` | No | Create poker game |
...
| `POST /api/games/{id}/action` | Yes | Submit action |
| `POST /api/games/{id}/resign` | Yes | Leave tournament |
| `POST /api/games/{id}/chat` | Yes | Send chat |
| `POST /api/games/{id}/extend` | Yes | Time extension |
| `GET /api/games/{id}/spectator` | No | Public view |
| `GET /api/games/{id}/escrow` | No | Escrow config (funded) |
| `GET /api/games/{id}/funding` | No | Deposit status (funded) |
| `GET /api/games/{id}/settlement` | No | Settlement data (funded) |
| `GET /api/games/{id}/offchain-settlement` | No | Off-chain payout report |
